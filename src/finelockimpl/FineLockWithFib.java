 ...
 * threads are spawned and fine grained synchronisation is done. Also fib(17) is
 * called before updating hash map
 * 
 * @author fibin
 *
 */
public class FineLockWithFib {

	// accumMapShared - HashMap that stores the accum sum and count per station.
	// Shared between
	// all threads
	Map<String, List<Integer>> accumMapShared = new HashMap<>();
	// dummyObj - A dummy object shared by all threads. In order to implement fine
	// lock,
	// we needed to put a temporary lock on some portion of the code.
	// instead of locking the entire data structure for whole sequence, we are fine
	// tuning
	// it to be two separate locks so that two threads can work on these
	// blocks(locked by separate locks),
	// parallely.
	// idea taken from:
	// https://stackoverflow.com/questions/41506470/synchronizing-by-this-vs-dummy-object
	...
	public Map<String, Float> getAvgTempPerStation(List<String> inputList) {
		...
		// boolean(isFibonacci) and dummy object shared between multiple threads
		Thread t1 = new Thread(new Runner(inputForThread1, 1, accumMapShared, true, dummyObj));
		Thread t2 = new Thread(new Runner(inputForThread2, 2, accumMapShared, true, dummyObj));
		Thread t3 = new Thread(new Runner(inputForThread3, 3, accumMapShared, true, dummyObj));
