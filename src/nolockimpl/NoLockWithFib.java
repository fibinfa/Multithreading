 ...
 * threads are spawned but no synchronization is done
 * 
 * @author fibin
 *
 */

public class NoLockWithFib {

	// accummap shared between three threads. All threads will update this data
	...
	public Map<String, Float> getAvgTempPerStation(List<String> inputList) {
		...
		// boolean(isFibonacci)
		Thread t1 = new Thread(new Runner(inputForThread1, 1, accumMapShared, true));
		Thread t2 = new Thread(new Runner(inputForThread2, 2, accumMapShared, true));
		Thread t3 = new Thread(new Runner(inputForThread3, 3, accumMapShared, true));
